...
COMMANDS = {
    "get_countries": {
        "description": "Lists all registered countries.",
        "args": [],
        "example": "get_countries",
    },
    "get_country": {
        "description": "Search for a country by its locality code.",
        "args": ["locale:str"],
...
async def handle_command(db: DatabaseProtocol, parts: list[str]) -> None:
    ...
    if cmd not in COMMANDS:
        print(f"Command from unknown country: {cmd}")
        return
    ...
    if cmd == "get_countries":
        ...
        if country:
            ...
            print(f"Country with code '{locale_code.upper()}' not found.")
