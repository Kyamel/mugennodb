   ...
   from mugennodb.database.interface.manga_review import insert_review


   COMMANDS = {
       "insert_review": {
           "description": "Create test manga review with default values",
           "args": ["--manga_id:str?", "--review_text:str?"],
           "example": "insert_review --manga_id=1 --review_text='Great manga!'",
       },
   }
   ...
   async def handle_command(db, parts: list[str]) -> None:
       ...
       if cmd == "insert_mangaReview":
           manga_id = args.get("manga_id", "1")
           review_id = args.get("review_id", "1")
           ...
           review_id = await insert_review(db, review)
