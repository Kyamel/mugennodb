...
async def insert_read(db: DatabaseProtocol, read: IRead) -> bool:
    ...
    record = await db.fetchrow(
        ...
        RETURNING *
        """,
        read.user_id, read.manga_id, read.status
    )
    return record_to_read(record) is not None


async def get_read(db: DatabaseProtocol, user_id: int, manga_id: int) -> Optional[IRead]:
    """
    Busca uma leitura pelo par (user_id, manga_id).
    """
    record = await db.fetchrow(
        "SELECT * FROM users_mangas WHERE user_id=$1 AND manga_id=$2",
        user_id, manga_id
    )
    return record_to_read(record)


async def update_read_status(db: DatabaseProtocol, user_id: int, manga_id: int, status: str) -> bool:
    """
...
async def delete_read(db: DatabaseProtocol, user_id: int, manga_id: int) -> bool:
    ...
    result = await db.execute(
        "DELETE FROM users_mangas WHERE user_id=$1 AND manga_id=$2",
        user_id, manga_id
    )
